...
import { 
  ...
  IconButton, TextField, CircularProgress, Typography, 
  useTheme, useMediaQuery, Avatar, InputAdornment
} from "@mui/material";
    ...
    <div className="w-full mx-auto flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <TextField
            ...
            className="w-full text-white sm:w-64 bg-white dark:bg-gray-800 rounded-lg"
            slotProps={{
              input: {
                startAdornment: (
          ...
          <Button 
            ...
            disableElevation
            startIcon={<Add />}
            className="whitespace-nowrap rounded-lg px-6 py-2 shadow-sm"
          >
            Client
          </Button>
        </div>
      ...
      {clients && clients.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-12 bg-gray-50 dark:bg-gray-800/50 rounded-3xl border-2 border-dashed border-gray-200 dark:border-gray-700">
          ...
          <Button onClick={() => setDialogState('add')} variant="outlined" startIcon={<Add />}>
            Add First Client
          </Button>
        </div>
            ...
            >
              <div className="flex items-center justify-between p-5 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm transition-all duration-200 hover:shadow-md hover:border-primary/30 active:scale-[0.98]">
                ...
                <div className="flex items-center gap-4">
                  <Avatar sx={{ bgcolor: theme.palette.primary.main, width: 48, height: 48, fontSize: '1rem', fontWeight: 600 }}>
                    {getInitials(client.firstName, client.lastName)}
                  </Avatar>
                  
                  <div className="flex flex-col">
                    <Typography variant="subtitle1" fontWeight="600" className="text-gray-900 dark:text-white leading-tight">
                ...
                <div className="flex items-center justify-center"> {/* Wrapped in a flex-center div */}
                  <IconButton 
                    size="small"
        ...
        {dialogState === 'add' && (
          <form onSubmit={handleAddClient} noValidate>
            ...
            <DialogActions sx={{ p: 3, pt: 2 }}>
              <Button onClick={handleCloseDialog} disabled={isSubmitting} className="text-gray-500 hover:bg-gray-100">Cancel</Button>
              <LoadingButton loading={isSubmitting} variant="contained" type="submit" disableElevation className="rounded-lg px-6">
                Save Client
              </LoadingButton>
            </DialogActions>
          </form>
          ...
          <>
            ...
            <DialogContent>
              <Typography className="text-gray-600">
                Are you sure you want to delete <strong className="text-gray-900">{selectedClient?.firstName} {selectedClient?.lastName}</strong>? This action cannot be undone and will permanently remove their profile.
              </Typography>
            </DialogContent>
            <DialogActions sx={{ p: 3, pt: 2 }}>
              <Button onClick={handleCloseDialog} disabled={isSubmitting} className="text-gray-500 hover:bg-gray-100">Cancel</Button>
              <LoadingButton loading={isSubmitting} variant="contained" color="error" disableElevation onClick={handleDeleteClient} className="rounded-lg px-6">
                Delete
              </LoadingButton>
            </DialogActions>
          </>
